...
import ReactPDF, { Image, Text, View, Page, Document, StyleSheet, renderToBuffer } from '@react-pdf/renderer';
...
export const InvoicePDF = async (transaction: Transaction) => {
  ...
  const InvoiceTitle = () => (
    <View style={styles.titleContainer}>
      <View style={styles.spaceBetween}>
        <Image style={styles.logo} src='http://localhost:3000/iconO4S-100x100.png' />
  ...
  const pdfBuffer = async () => {
    const staticDir = path.resolve(dirname, '../../../../../../public/invoice');
    await ReactPDF.renderToFile(
      ...
      </Document>, `${staticDir}/invoice_${reciept_data.invoice_no}.pdf`
    );

    const pdfPath = `${staticDir}/invoice_${reciept_data.invoice_no}.pdf`;
    const fileBuffer = await fs.promises.readFile(pdfPath);
    const blob = new Blob([fileBuffer], { type: 'application/pdf' });

    const formData = new FormData();
    formData.append('file', blob, `invoice_${reciept_data.invoice_no}.pdf`);

    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/api/invoice`,
      {
        method: 'POST',
        credentials: 'include',
        body: formData,
...
async function getBuffer(transaction: Transaction) {
  const payload = await createPayloadClient();
