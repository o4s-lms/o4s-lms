      ...
      {items?.map((item, idx) => (
        <li key={idx}>
          <NavLink
            ...
            className='block w-full border-l px-4 py-2 duration-150 hover:border-indigo-600 hover:text-gray-900'>
            {item?.name}
          </NavLink>
        </li>
      ))}
...
const SearchBox = ({ ...props }) => (
  <div className='relative w-full'>
    <svg
      ...
      viewBox='0 0 20 20'
      fill='currentColor'
      className='absolute inset-y-0 left-3 my-auto h-5 w-5 text-gray-400'>
      <path
        fillRule='evenodd'
    ...
    <input
      {...props}
      type='email'
      className='w-full rounded-lg border bg-transparent py-2 pl-12 pr-3 text-sm text-gray-500 shadow-sm outline-none ring-blue-600 duration-200 focus:ring-2'
    />
  </div>
);
...
const Sidebar = () => {
  ...
  return (
    <>
      <nav
        className="fixed left-0 top-0 z-40 h-full w-full space-y-8 overflow-auto border-r bg-white sm:w-80">
