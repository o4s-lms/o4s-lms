...
import content from "../../../cursos/curso";
import cursos from "../../../cursos/cursos.json";

const Curso = ({ getLesson }) => {
      ...
      <div className="mt-16 lg:mt-20">
        ...
        <div className="mx-auto mt-12 justify-between gap-8 lg:flex lg:max-w-screen-xl lg:px-8">
          ...
          <div className="hidden flex-none lg:block">
            <PlayList items={cursos} className="sticky top-3 w-full" />
          </div>
        </div>
      </div>
...
export async function getServerSideProps({ query }) {
  ...
  const getLesson = cursos.find((item) => item.slug == slug);
  return {
    props: {
      getLesson,
